#!/usr/bin/env /share/app/python-2.7.10/bin/python
# -*- coding: utf-8 -*-
...
------------------------
Created by Li Miao Version 1.0.1 (20140225)
------------------------
Modified by Li Zhixin Version 1.1.0 (20170301)
------------------------
Modified by Li Fuqiang Version 1.3 (20170825): 
   1) multiple cpus support when qsub jobs
   2) could use specified resource for each job: queue/machine(s) (ex. st.q/edico.q@cngb-edico-a23-1,edico.q@cngb-edico-a23-2), project_name (ex. edico)
   3) could run on test node (xcompute-*, cngb-xcompute-*), not just the login node(login-*)
   4) base on /hwfssz1/ST_MCHRI/DISEASE/DEV/pipeline/rdresearch/v2.0.0/software/pymonitor-1.1/monitor

Modified by Li Fuqiang Version 1.4 (20171226):
   1) change "/opt/python/bin/python" to "/share/app/python-2.7.10/bin/python"

Modified by Li Fuqiang Version 1.5 (20180614):
   1) add "-clear -binding linear:num_proc" to qsub

------------------------
Modified by Li Zhanqing Version 2.0 (20190110):
   1) add "--P1 " argument for qsub.
   2) add "--P2 " argument for qsub.
   3) add "--q1 " argument for qsub.
   4) add "--q2 " argument for qsub.
...
class configFile(object):
	...
	def __init__(self):
		...
		if needupdate:
			...
			if not self.cf.has_option('base', 'defaultP1'):
				self.cf.set('base', 'defaultP1', 'tumortest')
			if not self.cf.has_option('base', 'defaultP2'):     ####add by LZQ
				self.cf.set('base', 'defaultP2', 'tumortest')
			if not self.cf.has_option('base', 'defaultq1'):
				self.cf.set('base', 'defaultq1', 'st.q')
			if not self.cf.has_option('base', 'defaultq2'):
				self.cf.set('base', 'defaultq2', 'fpga2.q')
			...
			#LZX
			if not self.cf.has_option('base', 'num_proc'):
				self.cf.set('base', 'num_proc', '1')
	...
	def removePrj(self, prjname):
		...
		if os.path.exists(prjdb):
			...
			lockfilename = prjdb + ".lock"
			if os.path.exists(lockfilename): # lizhixin
				try:
...
class ZDatabase(object):
	...
	def Init(self, filename):
		try:
			logging.info("--FileStorage start...--")
			self.storage = FileStorage.FileStorage(filename, read_only=False)
			logging.info("--FileStorage() OK--")
		except:
			logging.warning("--FileStorage() Failed--")
			return 0
		else:
	...
	def Close(self):
		transaction.commit()
		self.conn.close()
		logging.info("--commit() and close() OK--")
	def Clean(self):
		self.db.pack()
...
class projectClass(Persistent):
	...
	def ImportTaskmonitor(self, file, addmode):
		...
		for line in list.readlines():
			...
			if re.match(r'^\s+', line):
				...
				exit()
			joblist = line.strip().split() # joblist is a single script:0.1G:4  #/ifs4/BC_RD/PROJECT/split/bwa_mem0.sh:0.1G
			for jobstring in joblist:
				...
				if not jobroot.has_key(jobarray[0]):
					...
					jobroot[jobarray[0]].memRequestedG = ''
					jobroot[jobarray[0]].num_proc = ''
					jobroot[jobarray[0]].param_q = ''
					jobroot[jobarray[0]].param_P = ''
					
					if len(jobarray) >= 2:
						for i in range(1, len(jobarray)):
							#get memory limit
							pmatch = re.search(r'([\d\.]+)(\w)', jobarray[i])
							if pmatch:
							...
							#get cpu number limit by LFQ
							pmatch = re.match(r'(\d+)cpu', jobarray[i], flags=re.IGNORECASE)
							if pmatch:
								jobroot[jobarray[0]].num_proc = int(pmatch.group(1))
							#set job wc_queue_list (-q) parameter by LFQ
							#pmatch = re.match(r'(.*).q', jobarray[i])
							#if pmatch:
								#jobroot[jobarray[0]].param_q = jobarray[i]
							#set job (-q and -P) parameter by LZQ
							pmatch = re.match(r'(\d+)q', jobarray[i], flags=re.IGNORECASE)
							if pmatch:
								if int(pmatch.group(1)) == 1:
									jobroot[jobarray[0]].param_q = optlist.opt_q1
									jobroot[jobarray[0]].param_P = optlist.opt_P1
								elif int(pmatch.group(1)) == 2:
									jobroot[jobarray[0]].param_q = optlist.opt_q2
									jobroot[jobarray[0]].param_P = optlist.opt_P2
							#set edico/supermem job project_name (-P) parameter by LFQ
							#if jobarray[i] == 'edico':
								#jobroot[jobarray[0]].param_P = jobarray[i]
							#elif jobarray[i] == 'st_supermem':
								#jobroot[jobarray[0]].param_P = jobarray[i]
					jobroot[jobarray[0]].memRequestedG = jobroot[jobarray[0]].memRequestedG or ConfigFileObj.getHandle().getint('base', 'defaultMemG')
					jobroot[jobarray[0]].num_proc = jobroot[jobarray[0]].num_proc or 1
	...
	def ImportQsubsge(self, file, addmode, linesep, resource):
		# LZX
		print "ImportQsubsge module don't support multi-CPU mode!!!"
		# exit(1)
	...
	def Update(self, NoCounter):
		logging.info("Update(self, %s)"%NoCounter)
		if self.allFinished:
			return
		self.checkDiskSpace()
		if self.DiskWarning == 1:
			...
			return
		logging.info("self.getStat(%s)"%NoCounter)
		self.getStat(NoCounter)
		#manually override.
		if self.submitEnabled:
			logging.info("self.submitNew()")
			self.submitNew()
	...
	def getStat(self, NoCounter):
		logging.info("getStat(self, %s)"%NoCounter)
		DrmaaSessionObj = drmaa.Session()
		DrmaaSessionObj.initialize()
		self.currentJobs = 0
		finishedJobs = 0
	...
	def submitNew(self):
		...
		jobroot = self.zroot['jobs']
		joblist = jobroot.keys()
		# logging.info("joblist: %s"%joblist)
		#check dependency and submit qualified jobs.
		for jobname in joblist:
			# jobname: /szhwfs1/ST_AGRIC/temp/gongjianhui/project/HumanEdit/WGS/test1/outdir/shell/LZX_1/161230_I136_FCHGK5HBBXX_L6_WHRDHUMouwUAHAAAB-52/merge_bam.sh
			# logging.info("jobname: %s"%jobname)
			job = jobroot[jobname]
			# logging.info("job: %s"%job) # job: <__main__.jobClass object at 0x7f32aeacdf50>
			# logging.info("job.jobid: %s"%job.jobid)
			if not job.jobid: # if this job is new
				finished = 1
				hold_jid = []
				for (jobname, jobstat) in job.Dependent.items():
					# logging.info("job.Dependent->jobname:%s, jobstat:%s"%(jobname, jobstat))
					#if the dependency is still running, put it to hold list.
					if jobstat == 2:
						hold_jid.append(jobroot[jobname].jobid)
					finished = finished * jobstat
				if finished:
					# logging.info("if finished")	
					if len(hold_jid):
					...
					if self.currentJobs < self.maxJobs:
						...
						# logging.info("job.Submit(DrmaaSessionObj)")	
		DrmaaSessionObj.exit()
...
class jobClass(Persistent):
	...
	def Submit(self, DrmaaSessionObj):
		# logging.info("start Submit(self, DrmaaSessionObj)...")	
		jt = DrmaaSessionObj.createJobTemplate()
		jt.remoteCommand = self.FileName
		jt.workingDirectory = os.path.dirname(self.FileName)
		# logging.info("FileName:%s, workingDirectory:%s"%(jt.remoteCommand, jt.workingDirectory))
		if self.param_q:
			self.projectObj.param_q = self.param_q
		elif optlist.opt_q1:
			self.projectObj.param_q = optlist.opt_q1
		elif optlist.opt_q2:
			self.projectObj.param_q = optlist.opt_q2
		else:
			self.projectObj.param_q = "st.q"
		if self.param_P:
			self.projectObj.param_P = self.param_P
		elif optlist.opt_P1:
			self.projectObj.param_P = optlist.opt_P1
		elif optlist.opt_P2:
			self.projectObj.param_P = optlist.opt_P2
		else:
			self.projectObj.param_P = "tumortest"
		#LZQ
		jt.nativeSpecification = "-b no -shell yes -l num_proc=" + str(self.num_proc) + ",vf=" + str(self.memRequestedG) + "G" + " -q " + self.projectObj.param_q + " -binding linear:" + str(self.num_proc) + " -P " + self.projectObj.param_P + " " + self.Parameter
		#jt.nativeSpecification = "-clear -cwd -l num_proc=" + str(self.num_proc) + ",vf=" + str(self.memRequestedG) + "g" + " -q " + self.projectObj.param_q + " -binding linear:" + str(self.num_proc) + " -P " + self.projectObj.param_P + " " + self.Parameter
		#LZX
		#jt.nativeSpecification = "-b no -shell yes -l vf=" + str(self.memRequestedG) + "G" + " -l num_proc=" + str(self.num_proc) + " -binding linear:" + str(self.num_proc) + " -P " + self.projectObj.param_P + " -q " + self.projectObj.param_q + " " + self.Parameter
		# jt.nativeSpecification = "-l vf=" + str(self.memRequestedG) + "G" + " -P " + self.projectObj.param_P + " -q " + self.projectObj.param_q + " " + self.Parameter
		#FIXME: prevent repeat submit
		# logging.info("jt.nativeSpecification:%s"%jt.nativeSpecification)
		previd = 0
		...
		if previd:
			...
			# bug fixed by Li Zhixin
			try:
				# logging.info("DrmaaSessionObj.runJob(jt)...")
				self.jobid = DrmaaSessionObj.runJob(jt)
			except:
				# a big bug fixed by Li Zhixin. 
				# drmaa can't submit -q "bc_rd.q", "_" is not allowed!!! 
				try:
					qsub_cmd = "qsub -clear -wd %s %s %s" % (jt.workingDirectory, jt.nativeSpecification, jt.remoteCommand)
					#LZQ
					#qsub_cmd = "qsub -clear -cwd %s %s %s" % (jt.workingDirectory, jt.nativeSpecification, jt.remoteCommand)
					qsub_cmd_list = qsub_cmd.split()
					# qsub_log = call(qsub_cmd_list)
					qsub_id = subprocess.Popen(qsub_cmd_list, stdout=subprocess.PIPE)
					#print stdout
					qsub_log = qsub_id.stdout.readline()
					qsub_job_id = qsub_log.strip().split()[2]
					self.jobid = qsub_job_id
				except:
					print("Error: Can't submit your jobs, please check your qsub parameters:\nqsub %s %s"%(jt.nativeSpecification,jt.remoteCommand))
					print("You can change your default values by 'monitor setdefault' command.")
					exit()
				finally:
					# pass
					logging.info("call(qsub_cmd_list) compelete!!")
			finally:
				# pass
				logging.info("DrmaaSessionObj.runJob(jt) compelete!!")
		if not self.jobid:
			self.jobid = 'unknown'
		self.projectObj.writeLog(self.FileName, self.jobid, "submitted")
		DrmaaSessionObj.deleteJobTemplate(jt)
		# logging.info("Submit(self, DrmaaSessionObj) compelete!!!")	
...
def usage():
	...
	Modified by Li Zhixin Version 1.1.0 (20170301)
	------------------------
	Modified by Li ZhanQing Version 1.2.0 (20190110)
	------------------------
	Usage:
	...
	Command List:
		taskmonitor	Add a task_monitor.py format task list.
			...
			--P1 <STR>  	-P1 argument for qsub.
			--P2 <STR> 	-P2 argument for qsub.
			--q1 <STR>	-q1 argument for qsub.
			--q2 <STR>	-q2 argument for qsub.
	...
	Document:
		The most recent document was placed in  http://bgird.genomics.cn/?p=1742
	"""
	print (usage.__doc__)
	exit(1)
...
def projectimport():
	...
	remove_symlink()
	# print(optlist.opt_q)
	logging.info("optlist.opt_q1 : %s, optlist.opt_P1 : %s" % (optlist.opt_q1, optlist.opt_P1))
	logging.info("optlist.opt_q2 : %s, optlist.opt_P2 : %s" % (optlist.opt_q2, optlist.opt_P2))
	if not optlist.opt_p:
		print "project name must be supplied."
		return
	...
	if socket.gethostname().find('compute') > -1:
		# LZX
		if socket.gethostname().startswith("xcompute") or socket.gethostname().startswith("cngb-xcompute"):
			print("running monitor on %s."%socket.gethostname())
		else:
			print "WARN: This program should be executed on login node."
			return
	...
	if confighandle.has_option('project', projectname):
		logging.info("--confighandle prj option exist--")
		...
		logging.info("--confighandle prj option not exist--")
		projectlocation = os.getcwd() + "/" + projectname + ".db"
		ConfigFileObj.addPrj(projectname, projectlocation)
	logging.info("--get projectlocation OK--\n" + "-"*20)
	logging.info("--zodbobj = ZDatabase() start...--")
	zodbobj = ZDatabase()
	if not zodbobj.Init(projectlocation):
		logging.warning("--zodbobj.Init() Failed--")
	...
	projectobj.param_P1 = optlist.opt_P1
	projectobj.param_P2 = optlist.opt_P2
	projectobj.param_q1 = optlist.opt_q1
	projectobj.param_q2 = optlist.opt_q2
	...
	if "taskmonitor" in args:
		logging.info("--start taskmonitor module--\n" + "-"*20)
		...
		logging.info("--start ImportTaskmonitor module--\n" + "-"*20)
...
def cronjob():
	...
	if optlist.opt_m == 1:
		...
		zodbobj = ZDatabase()
		for (name, location) in projectlist:
			logging.info("read conf...")
			logging.info("%s;%s"%(name, location))
			if not zodbobj.Init(location):
				logging.info("not zodbobj.Init(location)")
				continue
			zodbroot = zodbobj.Open()
			if len(zodbroot) > 0:
				logging.info("len(zodbroot) > 0")
				projectobj = zodbroot['projectobj']
				logging.info("start projectobj.Update(0)...")
				projectobj.Update(0)
				logging.info("projectobj.Update(0) finish!")
			zodbobj.Close()
			zodbobj.Uninit()
	elif optlist.opt_m == 2:
		logging.info("optlist.opt_m == 2")
		if cronobj.checkCron():
			return
		#submit email.
...
def setdefault():
	...
	if optlist.opt_P1:
		confighandle.set('base', 'defaultP1', optlist.opt_P1)
	if optlist.opt_P2:
		confighandle.set('base', 'defaultP2', optlist.opt_P2)
	if optlist.opt_q1:
		confighandle.set('base', 'defaultq1', optlist.opt_q1)
	if optlist.opt_q2:
		confighandle.set('base', 'defaultq2', optlist.opt_q2)
	if optlist.opt_n:
...
def remove_symlink():
    # repair a bug of ZODB, if ZODB were closed, there is no need to lock it any more.
    # bug found by Li Zhixin
    if confighandle.has_section('project'):
        for (name, location) in confighandle.items('project'):
            symlink_file = location + ".lock"
            if os.path.exists(symlink_file):
                os.remove(symlink_file)
                # print("os.remove(%s)"%symlink_file)
...
if __name__=='__main__':
	...
	#				datefmt='%d %b %H:%M:%S',
	#				filename = os.path.join(FILE,'pymonitor.log'),
	#				filemode='w')
	...
	confighandle = ConfigFileObj.getHandle()
	logging.info("--configFile() set complete--\n" + "-"*30)
	parser = OptionParser()
	parser.add_option("-p", dest = "opt_p", type = "string")
	parser.add_option("-m", dest = "opt_m", type = "int", default = 0)
#	parser.add_option("-q", dest = "opt_q", type = "int", default = confighandle.get('base', 'defaultq'))
	parser.add_option("--q1", dest = "opt_q1", type = "string", default = confighandle.get('base', 'defaultq1'))
	parser.add_option("--q2", dest = "opt_q2", type = "string", default = confighandle.get('base', 'defaultq2'))
#	parser.add_option("--P", dest = "opt_P", type = "int", default = confighandle.get('base', 'defaultP'))
	parser.add_option("--P1", dest = "opt_P1", type = "string", default = confighandle.get('base', 'defaultP1'))
	parser.add_option("--P2", dest = "opt_P2", type = "string", default = confighandle.get('base', 'defaultP2'))
	...
	logging.info("--OptionParser() set complete--\n" + "-"*30)
	logging.info("optlist.opt_q1 : %s, optlist.opt_P1 : %s" % (optlist.opt_q1, optlist.opt_P1))
	logging.info("optlist.opt_q2 : %s, optlist.opt_P2 : %s" % (optlist.opt_q2, optlist.opt_P2))
	# confighandle = ConfigFileObj.getHandle()
	ConfigFileObj.cf.set('base', 'defaultq1', optlist.opt_q1)
	ConfigFileObj.cf.set('base', 'defaultq2', optlist.opt_q2)
	ConfigFileObj.Update()
	...
	if 'help' in  args:
		...
		logging.info("--starting :::: projectimport()--")
		projectimport()
	elif 'qsubsge' in args:
		projectimport()
