...
def meregRawPSMandobsVSpredRTdiff(rawpsm, rtfile, prefix):
    ...
    # rawpsm_df['RTsecond.bak'] = rawpsm_df['RTsecond']
    # # 1.17780 -> 1.1
    # rawpsm_df['RTsecond'] = rawpsm_df['RTsecond'].apply(lambda x: float('.'.join([str(x).split('.')[0],str(x).split('.')[1][0:1]])))
    # ref_df['obsRTinSEC'] = ref_df['obsRTinSEC'].apply(lambda x: float('.'.join([str(x).split('.')[0],str(x).split('.')[1][0:1]])))
    # ref_df = ref_df.rename(columns={'obsRTinSEC':'RTsecond'})
    # rawpsm_df = pd.merge(rawpsm_df, ref_df, on=['Sequence','RTsecond'], how='left')
    # rawpsm_df = rawpsm_df.rename(columns={'predRTinSEC':prefix+'predRTinSEC', 'deltaRTinSEC':prefix+'deltaRTinSEC'})
    # rawpsm_df['RTsecond'] = rawpsm_df['RTsecond.bak']
    # rawpsm_df = rawpsm_df.drop('RTsecond.bak', axis=1)
