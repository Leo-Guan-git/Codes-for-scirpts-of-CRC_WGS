...
# CRC01.Fusion.Tumor.pep8.Trinity.orfipy.merge.detail.info
...
mutTypeNum = info_df.iloc[:, 11]
mutTypes = info_df.iloc[:, 12]
...
for i in range(len(info_df)):
    ...
    output.write(MUTPEPid2 + '\t' + str(mutAA[i]) + '\t' + str(int(mutAAlength[i])) + '\t' + str(int(mutSiteNum[i])) +
                 ...
                 '\t' + str(geneName[i]) + '\t' + str(mutTypeNum[i]) + '\t' + str(mutTypes[i]) +
                 '\t' + str(ORFInfo[i]) + '\n')
