...
echo -e "date\n\n${TrinityDir}/Trinity --genome_guided_bam ${workDir}/${sampleID}/${sampleType}/02.Aligna2Genome/STAR/run/Aligned.sortedByCoord.out.bam --genome_guided_max_intron 10000 --CPU 8 --max_memory 50G --output ${workDir}/${sampleID}/${sampleType}/03.${assembleModel}_Assemby/Trinity/trinity_out\n\ndate\n\nqstat -j \$JOB_ID | grep cpu" > ${assembleModel}_Assemby_Trinity.sh
...
echo -e "date\n\n${orfipyDir}/orfipy ${workDir}/${sampleID}/${sampleType}/03.${assembleModel}_Assemby/Trinity/trinity_out/Trinity-GG.fasta --start ATG,ATA,ATT,ATC,AAG,ACG,AGG,TTG,CTG,GTG --stop TAA,TAG,TGA --min 24 --outdir ${workDir}/${sampleID}/${sampleType}/04.${assembleModel}_ORF_Predict/orfipy --dna "${sampleID}.${sampleType}.${assembleModel}.orfipy.predicted.ORF.NT.fasta"\n\ndate\n\nqstat -j \$JOB_ID | grep cpu" > ${assembleModel}_ORFpredict_orfipy.sh
nohup bash ${assembleModel}_ORFpredict_orfipy.sh &

echo -e "date\n\n${orfipyDir}/orfipy ${workDir}/${sampleID}/${sampleType}/03.${assembleModel}_Assemby/Trinity/trinity_out/Trinity-GG.fasta --start ATG,ATA,ATT,ATC,AAG,ACG,AGG,TTG,CTG,GTG --stop TAA,TAG,TGA --min 24 --outdir ${workDir}/${sampleID}/${sampleType}/04.${assembleModel}_ORF_Predict/orfipy --pep "${sampleID}.${sampleType}.${assembleModel}.orfipy.predicted.ORF.pep.fasta"\n\ndate\n\nqstat -j \$JOB_ID | grep cpu" > ${assembleModel}_ORFpredict_pep_orfipy.sh
nohup bash ${assembleModel}_ORFpredict_pep_orfipy.sh &
